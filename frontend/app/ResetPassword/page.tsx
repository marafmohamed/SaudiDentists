...
export default function Page() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    //check each input field and update the state of the error
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    let errors: { email?: string; VerificationCode?: string } = {};
    if (!formData.email) {
    ...
    ResetPassword(formData.email).then((data: { error?: string }) => {
      console.log(data);
      if (data.error) {
        setErrors({ email: "بريد الكتروني غير موجود" });
        return;
      }
      setErrors({});
  ...
  const handleVerifieCode = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    let errors: { VerificationCode?: string; password?: string } = {};
    if (!formData.VerificationCode) {
      errors.VerificationCode = "كود التحقق مطلوب";
    }
    if (Object.keys(errors).length > 0) {
      setErrors(errors);
      return;
    ...
    VerifieCode(
      ...
      if (data.error == "New password is not strong enough") {
        setErrors({ password: "كلمة المرور ضعيفة" });
        return;
      } else if (data.error == "Invalid or expired OTP") {
        setErrors({ VerificationCode: "كود التحقق غير صحيح" });
        return;
      }
      ...
      {Verification && (
        ...
        >
          ...
          >
            ...
            />
            <svg
              onClick={togglePasswordVisibility} // Add onClick handler to toggle visibility
              viewBox="64 64 896 896"
              focusable="false"
              data-icon="eye-invisible"
              fill="currentColor"
              aria-hidden="true"
              className="size-6 cursor-pointer" // Add cursor pointer to indicate it's clickable
            >
          ...
          <Link
            href={"/Login"}
            className=" text-sm text-[#0c6780] w-96 text-right hover:no-underline transition-all hover:text-[#1890ff] cursor-pointer underline   font-bold tracking-tight underline-offset-2 "
          >
            العودة إلى تسجيل الدخول
          </Link>
        </form>
